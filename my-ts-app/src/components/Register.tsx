...
// 4) define the type of form data being sumitted by creating the user profile object
type userProfile = {
  email: string;
  password: string;
...
function Register() {
  // 5) link our form to useForm hook , useForm hook returns an object we call submittedFormValues
  ...
  // 6) create submittedFormValue object to destructure form values register, control, handleSubmit and formState
  ...
  // 7) destructure errors object, which contains individual field errors
  const { errors } = formState;

  // 8) define onsubmit function that should be called when the submit button is pressed
  ...
  return (
    <div>
      ...
      {/* 9) listen to the form onsubmit event and assign handleSubmit as a handler
          - to handleSubmit pass onsubmit as argument */}
      <form onSubmit={handleSubmit(onSubmit)}
        // 10) add noValidate attribute to the form element , prevents default browser validation 
        noValidate>
        ...
        <input
          ...
          {
          // 11) use register method to allow rhf to start tracking the state of form controls by adding props
          // on input tags eg email = {email}, simplify by spreading the register method on the form control
          ...register(
            "email",
            {
              // 12 ) add required field validation to the email field 
        ...
        />
        {/* 14 ) access field errors and display them below the corresponding element 
      ...
      {/* 3) invoke devtool after closing form tag
